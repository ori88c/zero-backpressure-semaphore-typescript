...
class ZeroBackpressureSemaphore {
    constructor(maxConcurrentJobs) {
        ...
        this._availableRoomExists = Promise.resolve(); // Resolved if-and-only-if an available room exists.
        this._notifyAvailability = null; // Resolving the above, if in pending-state.
    ...
    async _getAvailableRoom() {
        ...
        if (this._availableRoomsStack.length === 0) {
            this._availableRoomExists = new Promise(resolve => this._notifyAvailability = resolve);
        }
     ...
     */
    async _handleJobExecution(job, allottedRoom, isBackgroundJob) {
        try {
...
//# sourceMappingURL=ZeroBackpressureSemaphore.js.map